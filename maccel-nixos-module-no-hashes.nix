# NixOS Module for maccel - Mouse acceleration driver (Development version with no hashes)
...
let
  ...
  # Build the maccel kernel module using builtins.fetchGit (no hash required)
  maccel-kernel-module = config.boot.kernelPackages.callPackage (
    ...
    stdenv.mkDerivation rec {
      ...
      # Using builtins.fetchGit - no hash needed!
      src = builtins.fetchGit {
        url = "https://github.com/Gnarus-G/maccel.git";
        ref = "v${version}";
        # rev = "..."; # Optional: specify exact commit
      };
  ...
  # Build the maccel CLI and TUI tools using fake hash
  maccel-tools = pkgs.rustPlatform.buildRustPackage rec {
    ...
    # Use fake hash - Nix will tell you the correct one on first build
